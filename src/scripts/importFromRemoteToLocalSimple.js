...
async function importToLocalTable(tableName, data) {
  ...
  try {
    ...
    const { error: checkError } = await localSupabase
      .from(tableName)
      .select('*')
      .limit(1);
    
    if (checkError) {
      console.error(`La tabella ${tableName} non esiste nel database locale o non è accessibile`);
      console.log(`Provo a creare la tabella ${tableName}...`);
    ...
    try {
      const { error: deleteError } = await localSupabase
        .from(tableName)
        .delete()
        .gte('id', 0); // Condizione che dovrebbe essere vera per tutti i record
      
      if (deleteError && !deleteError.message.includes('no rows')) {
        console.error(`Errore nell'eliminazione dei dati esistenti:`, deleteError.message);
      } else {
        console.log(`Dati esistenti eliminati dalla tabella ${tableName}`);
      }
    } catch (deleteError) {
      console.error(`Errore durante l'eliminazione dei dati:`, deleteError.message);
    }
    
    // Importa i dati in batch per evitare problemi con set di dati grandi
    const BATCH_SIZE = 50;
    for (let i = 0; i < data.length; i += BATCH_SIZE) {
      ...
      if (insertError) {
        console.error(`Errore nell'importazione del batch ${i / BATCH_SIZE + 1}:`, insertError.message);
      } else {
        console.log(`Batch ${i / BATCH_SIZE + 1} importato con successo (${batch.length} record)`);
      }
    }
    
    console.log(`Importazione completata per la tabella ${tableName}`);
  } catch (error) {
    console.error(`Errore durante l'importazione nella tabella ${tableName}:`, error.message);
  }
}

/**
 * Funzione per creare una tabella basata sulla struttura dei dati
 */
async function createTable(tableName, sampleData) {
  if (!sampleData) return false;
  
  try {
    // Genera una definizione di tabella basata sui dati di esempio
    ...
    const { error } = await localSupabase.rpc('execute_sql', {
      query: `CREATE TABLE IF NOT EXISTS "${tableName}" (${columns});`
    });
    
    if (error) {
      console.error(`Errore nella creazione della tabella ${tableName}:`, error.message);
      return false;
    }
    ...
    console.error(`Errore durante la creazione della tabella ${tableName}:`, error.message);
    return false;
  }
}
